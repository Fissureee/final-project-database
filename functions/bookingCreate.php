...
$usersQuery = $conn->query("SELECT UserID, UserName FROM Users");
...
if (isset($_POST['addBooking'])) {
    ...
    if (strtotime($endBookingDate) < strtotime($bookingDate)) {
        ...
        header("Location: ../dashboard/dashboard_bookings.php?error=EndBookingDateEarlier");
        exit();
    }
    ...
    if (!empty($existingBookings)) {
        // Handle the error, for example, you can redirect back with an error message
        header("Location: ../dashboard/dashboard_bookings.php?error=VenueAlreadyBooked");
        exit();
    }

    // Perform the insertion for Bookings
    $insertBookingQuery = $conn->prepare("INSERT INTO Bookings (VenueID, UserID, BookingDate, EndBooking) VALUES (?, ?, ?, ?)");
    $insertBookingQuery->execute([$venueID, $userID, $bookingDate, $endBookingDate]);
